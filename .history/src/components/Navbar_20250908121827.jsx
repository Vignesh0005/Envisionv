...
import GrainSizeAnalysis from './GrainSizeAnalysis';
...
const Navbar = ({ imagePath, setImagePath, currentImageUrl, onClearWorkspace, addToImageHistory, onCalibrationClick }) => {
  ...
  const [showGrainSize, setShowGrainSize] = useState(false);
  ...
  const handleLowPassFilter = useCallback(async () => {
    try {
        if (!imagePath) {
  ...
  const handleMedianFilter = async () => {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/median-filter', {
            ...
            body: JSON.stringify({
                imagePath: imagePath,
                kernelSize: 15  // Default kernel size
            })
        });
  ...
  const handleEdgeDetect = async () => {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/edge-detect', {
            ...
            body: JSON.stringify({
                imagePath: imagePath,
                lowThreshold: 100,  // Default low threshold
                highThreshold: 200,  // Default high threshold
                blurKernel: 5  // Default blur kernel size
            })
        });
  ...
  const handleEdgeEmphasis = async () => {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/edge-emphasis', {
            ...
            body: JSON.stringify({
                imagePath: imagePath,
                strength: 1.0  // Default strength value
            })
        });
  ...
  const handleThreshold = async () => {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/threshold', {
            ...
            body: JSON.stringify({
                imagePath: imagePath,
                type: 'otsu',  // Default threshold type
                threshold: null  // Will use Otsu's automatic threshold
            })
        });
  ...
  const handleThin = async () => {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/thin', {
            ...
            body: JSON.stringify({
                imagePath: imagePath,
                method: 'skeleton'  // Default method (true skeletonization)
            })
        });
  ...
  const handleOptionClick = (option) => {
    if (option === "Clear") {
      ...
      setShowGrainSize(true);
    } else if (option === "Particle Analysis") {
      alert("Particle Analysis is coming soon!");
    } else if (option === "Graphite Classification") {
      alert("Graphite Classification is coming soon!");
    } else if (option === "Coating Thickness") {
      alert("Coating Thickness analysis is coming soon!");
    }
    setActiveDropdown(null);
  };
  ...
  return (
    <>
      ...
      {showGrainSize && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto">
          <div className="min-h-screen px-4 text-center">
            <div className="fixed inset-0" onClick={() => setShowGrainSize(false)}></div>
            <div className="inline-block w-full max-w-7xl my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              <div className="absolute right-0 top-0 pr-4 pt-4">
                <button
                  onClick={() => setShowGrainSize(false)}
                  className="text-gray-400 hover:text-gray-500 focus:outline-none"
                >
                  <span className="sr-only">Close</span>
                  <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <GrainSizeAnalysis 
                onClose={() => setShowGrainSize(false)}
                imagePath={imagePath} 
                imageUrl={currentImageUrl}
              />
            </div>
          ...
          <div className="min-h-screen px-4 text-center">
            ...
            <div className="inline-block w-full max-w-7xl my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              ...
              <FlakeAnalysis 
                imagePath={imagePath} 
                imageUrl={currentImageUrl} 
                onClose={() => setShowFlakeAnalysis(false)} 
              />
            </div>
          </div>
        </div>
          ...
          <div className="min-h-screen px-4 text-center">
            ...
            <div className="inline-block w-full max-w-7xl my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              ...
              <DendriticArmSpacing 
                imagePath={imagePath} 
                imageUrl={currentImageUrl} 
                onClose={() => setShowDendriticArmSpacing(false)} 
              />
            </div>
          </div>
        </div>
      ...
      {showDeCarburization && (
        <DeCarburization 
          imagePath={imagePath} 
          imageUrl={currentImageUrl} 
          onClose={() => setShowDeCarburization(false)} 
        />
      )}
