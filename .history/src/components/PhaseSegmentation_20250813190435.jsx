...
const PhaseSegmentation = ({ imagePath, imageUrl, onClose }) => {
    ...
    useEffect(() => {
        if (imageUrl) {
            console.log('PhaseSegmentation: imageUrl received:', imageUrl);
            console.log('PhaseSegmentation: imagePath received:', imagePath);
            setDisplayUrl(imageUrl);
        } else if (imagePath) {
            console.log('PhaseSegmentation: No imageUrl, constructing from imagePath:', imagePath);
            const constructedUrl = `http://localhost:5000/api/get-image?path=${encodeURIComponent(imagePath)}`;
            console.log('PhaseSegmentation: Constructed URL:', constructedUrl);
            setDisplayUrl(constructedUrl);
        } else {
            console.log('PhaseSegmentation: No imageUrl or imagePath provided');
            setDisplayUrl(null);
        }
    }, [imageUrl, imagePath]);
                            ...
                            {displayUrl ? (
                                ...
                                <div className="flex items-center justify-center h-[400px] bg-gray-50 rounded-lg">
                                    <div className="text-center text-gray-500">
                                        ...
                                        <div className="text-xs">Please select an image to analyze</div>
                                    </div>
                                </div>
                            )}
                        </div>
