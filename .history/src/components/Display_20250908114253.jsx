...
const Display = ({ 
  ...
  const handleUndo = useCallback(() => {
    ...
    // Then try to undo shape operations
      ...
      console.log('historyIndex > 0:', historyIndex > 0);
    }
  ...
  const handleRedo = useCallback(() => {
    ...
    console.log('imageHistoryIndex:', imageHistoryIndex);
    console.log('imageHistory.length:', imageHistory ? imageHistory.length : 0);
    console.log('handleImageRedo exists:', !!handleImageRedo);
    ...
    if (imageHistory && imageHistory.length > 0 && imageHistoryIndex < imageHistory.length - 1 && handleImageRedo) {
      console.log('Redoing image operation');
      handleImageRedo();
    }
    // Then try to redo shape operations
    else if (historyIndex < history.length - 1) {
      console.log('Redoing shape operation');
      const newIndex = historyIndex + 1;
      setHistoryIndex(newIndex);
      onShapesUpdate(history[newIndex]);
    } else {
      console.log('Nothing to redo');
      console.log('imageHistory.length > 0:', imageHistory ? imageHistory.length > 0 : false);
      console.log('imageHistoryIndex < imageHistory.length - 1:', imageHistory ? imageHistoryIndex < imageHistory.length - 1 : false);
      console.log('historyIndex < history.length - 1:', historyIndex < history.length - 1);
    }
      ...
      <div className="absolute top-2 right-2 flex gap-2 z-10">
        <button
          ...
          onClick={() => {
            ...
            console.log('Button state - history.length:', history.length);
            handleUndo();
          }}
