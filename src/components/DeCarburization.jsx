...
const DeCarburization = ({ onClose, imagePath, imageUrl }) => {
  ...
  const handleEnterReportDetails = () => {
    if (results.length === 0) {
      message.warning('Please add some results before entering report details');
      return;
    }
    message.info('Report details form would open here');
  };

  const handleGenerateReport = () => {
    if (results.length === 0) {
      message.warning('Please add some results before generating report');
      return;
    }
    
    const reportData = {
      totalFields: results.length,
      totalDepth: results.reduce((sum, r) => sum + r.totalDepth, 0),
      averageMean: Math.round(results.reduce((sum, r) => sum + r.mean, 0) / results.length),
      averageStdDev: Math.round(results.reduce((sum, r) => sum + r.stdDev, 0) / results.length),
      results: results
    };
    
    console.log('De-Carburization Report Data:', reportData);
    message.success('Report generated successfully! Check console for data.');
  };
      ...
      <div className="bg-gray-50 border rounded p-4 mb-4 flex flex-wrap gap-4 items-end">
        <div className="flex flex-col text-xs mr-4">
          ...
          <div className="flex gap-2 mb-1">
            ...
            <input className="w-16 border rounded px-1 py-0.5 text-xs" readOnly value={measurements.max} />
          </div>
          <div className="flex gap-2 mb-1">
            <label className="w-20">Mean</label>
            <input className="w-16 border rounded px-1 py-0.5 text-xs" readOnly value={measurements.mean} />
          </div>
        </div>
        <div className="flex flex-col text-xs mr-4">
          <div className="flex gap-2 mb-1">
            <label className="w-20">Total Depth</label>
            <input className="w-16 border rounded px-1 py-0.5 text-xs" readOnly value={measurements.totalDepth} />
          </div>
          <div className="flex gap-2 mb-1">
            <label className="w-20">Total Count</label>
            <input className="w-16 border rounded px-1 py-0.5 text-xs" readOnly value={measurements.totalCount} />
          </div>
          <div className="flex gap-2 mb-1">
            <label className="w-20">Std Dev</label>
            <input className="w-16 border rounded px-1 py-0.5 text-xs" readOnly value={measurements.stdDev} />
          </div>
        </div>
      ...
      <div className="mb-4">
        <div className="border rounded bg-white h-40 flex flex-col">
          <div className="flex-1 flex items-center justify-center text-gray-400 text-xs">
            {results.length > 0 ? (
              <div className="text-center">
                <div className="text-lg font-semibold text-blue-600">{results.length} Fields Analyzed</div>
                <div className="text-sm text-gray-500 mt-1">
                  Total Depth: {results.reduce((sum, r) => sum + r.totalDepth, 0)} μm
                </div>
                <div className="text-sm text-gray-500">
                  Average Mean: {Math.round(results.reduce((sum, r) => sum + r.mean, 0) / results.length)} μm
                </div>
              </div>
            ) : (
              <div className="text-center">
                <div className="text-lg font-semibold text-gray-400">No Data</div>
                <div className="text-sm text-gray-400 mt-1">Add results to see chart data</div>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-center gap-4 mt-4">
            <Button size="large" onClick={handleEnterReportDetails}>Enter Report Details</Button>
            <Button size="large" type="primary" onClick={handleGenerateReport}>Report</Button>
          </div>
        </div>
      </div>
    </div>
