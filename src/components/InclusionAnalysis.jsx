...
const InclusionAnalysis = ({ imagePath, imageUrl, onClose }) => {
  ...
  const loadImage = (url) => {
    console.log('InclusionAnalysis: loadImage called with URL:', url);
    
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = () => {
      console.log('InclusionAnalysis: Image loaded successfully:', { width: img.naturalWidth, height: img.naturalHeight });
      setError(null);
      ...
      if (canvasRef.current) {
        ...
        console.log('InclusionAnalysis: Image drawn to canvas successfully');
      }
    };
    img.onerror = (error) => {
      console.error('InclusionAnalysis: Error loading image:', error);
  ...
  const handleGetImage = () => {
    console.log('InclusionAnalysis: handleGetImage called');
    console.log('InclusionAnalysis: Current displayUrl:', displayUrl);
  ...
  const handleAddToSummary = () => {
    ...
    message.success('Results added to summary');
  };

  const handleRunImagePrep = () => {
    if (!imagePrep) {
      message.warning('Please select an image preparation method first');
      return;
    }
    message.success(`Image preparation "${imagePrep}" completed`);
  };
        ...
        <div className="w-72 flex flex-col gap-2">
          <div className="bg-white border rounded p-3 flex flex-col gap-2">
            <div className="flex items-center gap-2">
              ...
              <Button size="small" onClick={handleRunImagePrep}>Run</Button>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="small" onClick={handleFindInclusion} loading={loading}>Find Inclusion</Button>
            </div>
          </div>
          <div className="bg-white border rounded p-3 flex flex-col gap-2">
            <div className="flex items-center gap-2">
      ...
      <div className="border-t bg-white px-4 pt-2 pb-4">
        <Tabs activeKey={bottomTab} onChange={setBottomTab} size="small">
                ...
                <tbody>
                  {['A', 'B', 'C', 'D'].map(type => (
                    <tr key={type}>
                      ...
                      <td className="border px-2 py-1">{currentResults[type].thin}</td>
                      <td className="border px-2 py-1">{currentResults[type].thick}</td>
                      <td className="border px-2 py-1">{currentResults[type].thin + currentResults[type].thick}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <TabPane tab="Over Sized" key="oversized">
            <div className="p-2">
              <table className="w-full text-xs border">
                ...
                <tbody>
                  {oversizeRows.length === 0 ? (
                    <tr><td colSpan={4} className="text-center text-gray-400">No oversized inclusions detected</td></tr>
                  ) : (
                    oversizeRows.map((row, i) => (
                      <tr key={i}>
                        <td className="border px-2 py-1">{row.id}</td>
          ...
          <TabPane tab="Method D Result" key="methodD">
            <div className="p-2">
              <div className="text-xs text-gray-600 mb-2">Method D Analysis Results</div>
              <table className="w-full text-xs border">
                <thead className="bg-gray-100">
                  <tr>
                ...
                <tbody>
                  <tr><td className="border px-2 py-1">Total Inclusions</td><td className="border px-2 py-1">{Object.values(currentResults).reduce((sum, type) => sum + type.thin + type.thick, 0)}</td></tr>
                  <tr><td className="border px-2 py-1">Field Area</td><td className="border px-2 py-1">{fieldArea} mm²</td></tr>
                  <tr><td className="border px-2 py-1">Density</td><td className="border px-2 py-1">{fieldArea > 0 ? (Object.values(currentResults).reduce((sum, type) => sum + type.thin + type.thick, 0) / fieldArea).toFixed(2) : 0} /mm²</td></tr>
                </tbody>
              </table>
            </div>
          </TabPane>
          <TabPane tab="Summary Data" key="summary">
            <div className="p-2">
              <div className="text-xs text-gray-600 mb-2">Summary of All Analyses</div>
              {summaryResults.length === 0 ? (
                <div className="text-xs text-gray-400">No summary data available. Click "Add" in Current Result to add data.</div>
              ) : (
                <table className="w-full text-xs border">
                  <thead className="bg-gray-100">
                    <tr>
                      <th className="border px-2 py-1">Specimen</th>
                      <th className="border px-2 py-1">Type A</th>
                      <th className="border px-2 py-1">Type B</th>
                      <th className="border px-2 py-1">Type C</th>
                      <th className="border px-2 py-1">Type D</th>
                      <th className="border px-2 py-1">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {summaryResults.map((result, i) => (
                      <tr key={i}>
                        <td className="border px-2 py-1">{result.specimen}</td>
                        <td className="border px-2 py-1">{result.A.thin + result.A.thick}</td>
                        <td className="border px-2 py-1">{result.B.thin + result.B.thick}</td>
                        <td className="border px-2 py-1">{result.C.thin + result.C.thick}</td>
                        <td className="border px-2 py-1">{result.D.thin + result.D.thick}</td>
                        <td className="border px-2 py-1">{result.A.thin + result.A.thick + result.B.thin + result.B.thick + result.C.thin + result.C.thick + result.D.thin + result.D.thick}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
