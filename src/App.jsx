...
const App = () => {
  ...
  const addToImageHistory = (newImagePath, operation) => {
    console.log('=== addToImageHistory called ===');
    console.log('newImagePath:', newImagePath);
    console.log('operation:', operation);
    console.log('Current imageHistory:', imageHistory);
    console.log('Current imageHistoryIndex:', imageHistoryIndex);
    ...
    console.log('=== addToImageHistory completed ===');
  };

  const handleImageUndo = () => {
    console.log('=== handleImageUndo called ===');
    console.log('imageHistory:', imageHistory);
    console.log('imageHistory.length:', imageHistory.length);
    ...
    if (imageHistory.length > 0 && imageHistoryIndex > 0) {
      ...
      console.log('New image path:', imageHistory[newIndex].path);
      setImageHistoryIndex(newIndex);
      setImagePath(imageHistory[newIndex].path);
      console.log('=== handleImageUndo completed ===');
    } else {
      console.log('Cannot undo image - no history or at beginning');
      console.log('imageHistory.length > 0:', imageHistory.length > 0);
  ...
  const handleClearWorkspace = () => {
    if (window.confirm('Are you sure you want to clear everything in the workspace? This will remove the current image, all measurements, and analysis results.')) {
      ...
      // Hide camera filters when clearing workspace
  ...
  const handleCameraFiltersClick = () => {
    setShowCameraFilters(!showCameraFilters);
  };
  ...
  const handleImageSelect = (imagePath) => {
    ...
    // Hide camera filters when an image is selected/captured
    setShowCameraFilters(false);
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col w-full z-50">
        <Navbar 
          ...
          onCameraFiltersClick={handleCameraFiltersClick}
        />
        {!(showPhaseSegmentation || showInclusion || showPorosity) && !showCameraFilters && (
          <Toolbar 
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col h-full" style={{ width: SIDEBAR_WIDTH, minWidth: SIDEBAR_WIDTH }}>
          {showInlineCalibration ? (
            ...
            <>
              {/* Drawn Shapes Panel or Camera Filters - fill sidebar up to ControlBox */}
              <div className="flex-1 min-h-0 w-full">
                {showCameraFilters ? (
                  <CameraFilters onClose={() => setShowCameraFilters(false)} />
                ) : (
                  <ShapeTracker
                    shapes={shapes}
